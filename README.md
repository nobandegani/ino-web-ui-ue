...
> **You normally don't need to run this.** `Source/ThirdParty/WebView2/` is
> **committed** to this repository — the two headers, `WebView2LoaderStatic.lib`,
> and a `VERSION` stamp — so a fresh clone builds without it. Run the script only
> to move the SDK version. (Earlier revisions of this README and `CLAUDE.md` said
> the folder was gitignored; it is not.)
>
> Those files are Microsoft's and are **not** covered by this plugin's licence —
...
**InoWebUI is licensed under the Apache License 2.0** — see [LICENSE](LICENSE).
The short version:

- **You can** use InoWebUI in any project, commercial or otherwise, free of
  charge.
- **You can** ship closed-source games that embed it. Apache-2.0 imposes no
  copyleft; your game code is entirely unaffected, and you are not obliged to
  publish your modifications.
- **You must** reproduce the contents of [NOTICE](NOTICE) — which is the
  attribution requirement, and it is binding. See below.
- You also get an explicit patent grant (Apache-2.0 §3), which MPL-2.0 gives
  more narrowly and MIT/BSD not at all.

### Required attribution

Apache-2.0 **§4(d)** requires that a Derivative Work reproduce the `NOTICE`
contents, including *"within a display generated by the Derivative Works, if
and wherever such third-party notices normally appear."* For a game, that
display is your credits or about screen. A line equivalent to this satisfies
it:

> Native WebView overlay powered by InoWebUI
> https://github.com/nobandegani/ino-web-ui-ue (Apache-2.0)

<details>
<summary>Why this changed from "MPL-2.0 plus an attribution requirement"</summary>

Earlier releases stated MPL-2.0 with an extra attribution clause appended to
the `LICENSE` file. That did not work as intended, for three reasons:

1. **MPL-2.0 cannot carry it.** A recipient of the Source Code Form under
   MPL-2.0 may redistribute it under MPL-2.0 alone; the appended condition
   could not bind them, so the requirement was unenforceable while still
   appearing mandatory.
2. **The URL was dead** — it pointed at `github.com/nobandegani/InoWebUI`,
   which is not this repository.
3. **No source file carried an MPL notice.** All 30 said "All Rights
   Reserved", which cannot coexist with an open-source grant, and MPL's
   copyleft attaches per file via that notice.

Apache-2.0's `NOTICE` mechanism does exactly what the original clause was
reaching for — mandatory, standard, and actually enforceable — so the
attribution intent is preserved and is now real.
</details>
